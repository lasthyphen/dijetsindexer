...
import (
	...
	dijethType "github.com/lasthyphen/dijeth/core/types"
	"github.com/lasthyphen/dijetsindexer/cfg"
	"github.com/lasthyphen/dijetsindexer/db"
	"github.com/lasthyphen/dijetsindexer/models"
	"github.com/lasthyphen/dijetsindexer/modelsc"
	"github.com/lasthyphen/dijetsindexer/services"
	"github.com/lasthyphen/dijetsindexer/services/indexes/params"
	"github.com/lasthyphen/dijetsindexer/servicesctrl"
	"github.com/lasthyphen/dijetsindexer/utils"
...
func (r *Reader) CTxDATA(ctx context.Context, p *params.TxDataParam) ([]byte, error) {
	...
	type BlockExport struct {
		...
		Header         dijethType.Header        `json:"header"`
		Uncles         []dijethType.Header      `json:"uncles"`
		TxsBytes       [][]byte                 `json:"txs"`
		Version        uint32                   `json:"version"`
		BlockExtraData []byte                   `json:"blockExtraData"`
		BlockExtraID   string                   `json:"blockExtraID"`
		Txs            []dijethType.Transaction `json:"transactions,omitempty"`
		Logs           []dijethType.Log         `json:"logs,omitempty"`
	}
	...
	block.Txs = make([]dijethType.Transaction, 0, len(rowsData))
	for _, rowData := range rowsData {
		var tr dijethType.Transaction
	...
	block.Logs = make([]dijethType.Log, 0, len(rowsData))
	for _, rowData := range rowsLog {
		var clog dijethType.Log
