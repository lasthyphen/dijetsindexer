...
WORKDIR /go/src/github.com/lasthyphen/dijetsindexer
...
COPY --from=builder /go/src/github.com/lasthyphen/dijetsindexer/docker/config.json /opt/config.json
COPY --from=builder /go/src/github.com/lasthyphen/dijetsindexer/services/db/migrations /opt/migrations
